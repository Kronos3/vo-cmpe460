...
namespace Os {
    ...
    bool BufferQueue::initialize(NATIVE_UINT_TYPE depth, NATIVE_UINT_TYPE msgSize) {
        U8* data = static_cast<U8*>(CMemorySystem::HeapAllocate(depth * (sizeof(msgSize) + msgSize), HEAP_ANY));
        if (NULL == data) {
            return false;
        }
        FIFOQueue* fifoQueue = static_cast<FIFOQueue*>(CMemorySystem::HeapAllocate(sizeof(FIFOQueue), HEAP_ANY));
        if (NULL == fifoQueue) {
            return false;
        }
        fifoQueue->data = data;
