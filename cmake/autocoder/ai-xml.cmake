set(AUTOCODER_NAME autocoder/ai_xml)

# Import the autocoder implementation
include(autocoder/ai-xml-proto)
...
####
function(ai_xml_run_ac SOURCES)
    set(AUTOCODER_NAME "ai_xml")
    init_variables(MODULE_DEPENDENCIES GENERATED_FILES CONSUMED_SOURCES)
    __ai_xml_run_ac("${SOURCES}" "${GENERATED_FILE_LIST}" "${INFO_ONLY}")
...
####
function(__ai_xml_memoize SOURCES)
    set(SOURCES_HASH "multiple")
    if(ai_xml_HANDLES_INDIVIDUAL_SOURCES)
        string(MD5 SOURCES_HASH "${SOURCES}")
    endif()
    set(MEMO_FILE "${CMAKE_CURRENT_BINARY_DIR}/${AUTOCODER_NAME}.${SOURCES_HASH}.dep")

    ai_xml_regenerate_memo(FORCE_REGENERATE "${MEMO_FILE}" "${SOURCES}")
    if(FORCE_REGENERATE)
        ai_xml_get_generated_files("${SOURCES}")
        ai_xml_get_dependencies("${SOURCES}")
...
####
function(__ai_xml_run_ac SOURCES GENERATED_SOURCES INFO_ONLY)
    ...
    if(HANDLES_INDIVIDUAL_SOURCES)
        ...
        foreach(SOURCE IN LISTS AC_INPUT_SOURCES)
            __ai_xml_ac_process_sources("${SOURCE}" "${INFO_ONLY}")
        ...
        __ai_xml_ac_process_sources("${AC_INPUT_SOURCES}" "${INFO_ONLY}")
        if(GENERATED_FILES)
...
endfunction(__ai_xml_run_ac)

####
# __ai_xml_ac_process_sources:
#
...
####
function(__ai_xml_ac_process_sources SOURCES INFO_ONLY)
    # Run the autocode setup process now with memoization
    __ai_xml_memoize("${SOURCES}")
    set(MODULE_DEPENDENCIES "${MODULE_DEPENDENCIES}" PARENT_SCOPE)
    set(GENERATED_FILES "${GENERATED_FILES}" PARENT_SCOPE)

    # Run the generation setup when not requesting "info only"
    if(NOT INFO_ONLY)
        ai_xml_setup_autocode("${SOURCES}" "${GENERATED_FILES}" "${MODULE_DEPENDENCIES}" "${FILE_DEPENDENCIES}" "${EXTRAS}")
    endif()
endfunction()
