set(AUTOCODER_NAME autocoder/fpp)

# Import the autocoder implementation
include(autocoder/fpp-proto)
...
####
function(fpp_run_ac SOURCES)
    set(AUTOCODER_NAME "fpp")
    init_variables(MODULE_DEPENDENCIES GENERATED_FILES CONSUMED_SOURCES)
    __fpp_run_ac("${SOURCES}" "${GENERATED_FILE_LIST}" "${INFO_ONLY}")
...
####
function(__fpp_memoize SOURCES)
    set(SOURCES_HASH "multiple")
    if(fpp_HANDLES_INDIVIDUAL_SOURCES)
        string(MD5 SOURCES_HASH "${SOURCES}")
    endif()
    set(MEMO_FILE "${CMAKE_CURRENT_BINARY_DIR}/${AUTOCODER_NAME}.${SOURCES_HASH}.dep")

    fpp_regenerate_memo(FORCE_REGENERATE "${MEMO_FILE}" "${SOURCES}")
    if(FORCE_REGENERATE)
        fpp_get_generated_files("${SOURCES}")
        fpp_get_dependencies("${SOURCES}")
...
####
function(__fpp_run_ac SOURCES GENERATED_SOURCES INFO_ONLY)
    ...
    if(HANDLES_INDIVIDUAL_SOURCES)
        ...
        foreach(SOURCE IN LISTS AC_INPUT_SOURCES)
            __fpp_ac_process_sources("${SOURCE}" "${INFO_ONLY}")
        ...
        __fpp_ac_process_sources("${AC_INPUT_SOURCES}" "${INFO_ONLY}")
        if(GENERATED_FILES)
...
endfunction(__fpp_run_ac)

####
# __fpp_ac_process_sources:
#
...
####
function(__fpp_ac_process_sources SOURCES INFO_ONLY)
    # Run the autocode setup process now with memoization
    __fpp_memoize("${SOURCES}")
    set(MODULE_DEPENDENCIES "${MODULE_DEPENDENCIES}" PARENT_SCOPE)
    set(GENERATED_FILES "${GENERATED_FILES}" PARENT_SCOPE)

    # Run the generation setup when not requesting "info only"
    if(NOT INFO_ONLY)
        fpp_setup_autocode("${SOURCES}" "${GENERATED_FILES}" "${MODULE_DEPENDENCIES}" "${FILE_DEPENDENCIES}" "${EXTRAS}")
    endif()
endfunction()
