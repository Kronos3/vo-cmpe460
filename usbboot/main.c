        ...
        libusb_context* ctx, uint16_t vendor_id)
{
    ...
    while ((dev = devs[i++]) != NULL)
    {
        ...
        */
        ...
        if (verbose == 2)
        {
            printf("Found device %lu idVendor=0x%04x idProduct=0x%04x\n", i, desc.idVendor, desc.idProduct);
...
FILE* check_file(const char* dir, const char* fname, int use_fmem)
{
    ...
    if (fp == NULL && use_fmem)
    {
        ...
        else
        {
            ...
            else if (strcmp(fname, "kernel.img") == 0 && kernel_img)
            {
                fp = fopen(kernel_img, "rb");
                if (fp)
                    printf("Found kernel image: %s\n", kernel_img);
            }
        }
        if (fp)
